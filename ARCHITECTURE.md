...
- **Email.** Workspace-owned channel accounts are the primary sender: connected Outlook / Microsoft 365 inboxes for founder-led and early GTM sends, with strict daily ceilings, native conversation threading, reply sync, and account health. Managed owned-domain sending (secondary customer domains, warmup, SPF/DKIM/DMARC, bounce/complaint feedback) is an optional capacity layer, not a launch dependency and not sent from Bombsell's brand domain. Gmail can be added later if the product accepts Google's restricted-scope verification burden; until then, Microsoft Graph is the default connected-inbox path.
...
| LLMs | **DeepSeek V4 Flash** as the single default for drafting, classification, light reasoning, hot-path judges, optimizer loops, and dedup. **DeepSeek V4 Pro** is reserved for explicit escalations such as hard multi-document synthesis, repeated Flash failure, or operator-approved investigations. **BYO** Anthropic / OpenAI keys remain possible for workspaces that want them. | One vendor, one billing relationship, one set of guardrails, and low default cost. Flash pricing absorbs the "judge every generation" workload; Pro stays available without becoming the hidden baseline. Provider-agnostic LLM client in `core/agents/llm/` keeps swap-out cheap. |
...
| Email infra | **Resend** for transactional product mail; **Outlook / Microsoft Graph** as the primary customer-connected outbound path; optional managed outbound adapters such as **Smartlead/Instantly** for mailbox rotation or **MailerSend/Mailgun/Azure Communication Services** for customer-owned sending domains | Separate Bombsell transactional reputation from customer prospecting reputation. Customer outbound should come from customer-controlled accounts/domains; platform-owned SES is a legacy optional adapter, not the product spine or a launch blocker. |
...
| Deploy | **Vercel** for Next, **Railway / Render / Fly** for workers + Restate; ECS Express is acceptable when stable | Don't trap workers in Vercel's request model, and don't let AWS hosting friction become product architecture. |
