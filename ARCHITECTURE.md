...
| Email infra | **Resend/Postmark/MailerSend** for transactional product mail; **Outlook / Microsoft Graph** as the primary customer-connected outbound path; optional managed outbound adapters such as **Smartlead** for mailbox rotation or **Mailgun/MailerSend/SES** for customer-owned sending domains | Separate Bombsell transactional reputation from customer prospecting reputation. Customer outbound should come from customer-controlled accounts/domains; platform-owned SES is optional capacity, not the product spine. |
