...
- **Tools are MCP servers, always.** Gmail, LinkedIn, Apollo, HubSpot, Stripe — every integration is an MCP server (mostly hosted by us, some BYO). Agents discover tools dynamically. This makes the same agent runnable from our UI, Claude desktop, ChatGPT, or a customer's own agent framework.
...
4. **Stress the trust layer**: enumerate every failure mode (LLM hallucinates a name, ZeroBounce is down, Gmail token expires mid-send, two Reps target the same person, a competitor signals churn on a customer). The design must have an answer for each — usually a typed defer event back onto the bus.
