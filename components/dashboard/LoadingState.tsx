...
export function DashboardLoadingState({
  ...
  return (
    <div className="space-y-8" aria-busy="true" aria-live="polite">
      <section className="dashboard-loader-panel relative overflow-hidden rounded-[10px] border border-[var(--color-line-1)] bg-[var(--color-ink-0)] px-6 py-10 md:px-8 md:py-12">
        <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--color-text-3)]">
          {copy.kicker}
        </p>
        <div className="mt-5 flex items-center gap-3">
          <span className="dashboard-loader-mark grid size-11 place-items-center rounded-[9px] bg-[var(--color-cta-bg)] text-[var(--color-cta-text)]">
            <Icon name={copy.icon} size={18} />
          </span>
          <h1
            className="text-[26px] font-semibold leading-tight tracking-[0] text-[var(--color-text-1)] md:text-[30px]"
          >
            {copy.title}
          </h1>
        </div>
