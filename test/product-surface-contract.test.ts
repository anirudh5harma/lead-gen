...
test("Dashboard routes setup work through Profile and current surfaces", () => {
  ...
  assert.match(dashboard, /BriefSnapshotPanel/);
  assert.match(dashboard, /BriefWindowMetricRow/);
  assert.match(dashboard, /ReplySnapshotCard/);
  assert.match(dashboard, /Last day and week/);
  ...
  assert.match(dashboard, /value="\/dashboard\/agent#review-queue"/);
  ...
  assert.match(dashboard, /configure CRM in Profile/);
  assert.match(dashboard, /href: "\/dashboard\/profile#crm-sync"/);
  ...
  assert.match(dashboard, /href: "\/dashboard\/agent#review-queue"/);
  ...
  assert.match(dashboard, /BriefChannelReadiness/);
  ...
  assert.match(dashboard, /Outlook or LinkedIn must be connected/);
  assert.match(dashboard, /Connect LinkedIn/);
  assert.match(dashboard, /LinkedIn unlocks profile-backed connection requests/);
  ...
  assert.match(dashboard, /href: "\/dashboard\/profile#linkedin"/);
  ...
  assert.match(dashboard, /BriefLearningPanel/);
  assert.match(dashboard, /OutcomeInsightRow/);
  assert.match(dashboard, /href=\{`\/dashboard\/agent\/contacts\/\$\{contact\.id\}`\}/);
...
test("Profile presents separate Outlook and LinkedIn connection gates", () => {
  ...
  assert.match(settings, /source_kinds/);
  assert.match(settings, /qualified_signals_7d/);
...
test("Agent surface shows live work and account readiness", () => {
  ...
  assert.doesNotMatch(reps, /Source strategy/);
  assert.doesNotMatch(reps, /Readiness gate/);
  assert.match(reps, /System status/);
  assert.match(reps, /Get started/);
  assert.match(reps, /One path from Profile to signals, verified contacts, judged\s+outreach, and replies/);
  assert.match(reps, /Outreach gated/);
  assert.doesNotMatch(reps, /Qualified Signals can become email or LinkedIn outreach/);
  assert.doesNotMatch(reps, /readiness\.checks\.map/);
  ...
  assert.match(reps, /linkedin_only/);
  assert.match(reps, /fit_reviewed/);
  assert.match(reps, /needs_fit_review/);
  assert.match(reps, /blocked_by_fit/);
  assert.match(reps, /in_outreach/);
  assert.match(reps, /draft_ready/);
  assert.match(reps, /contacted/);
  assert.match(reps, /replied/);
  assert.match(reps, /Contact lanes/);
  assert.match(reps, /ContactLaneRow/);
  assert.match(reps, /Email-ready/);
  assert.match(reps, /LinkedIn-only/);
  assert.match(reps, /Needs fit review/);
  assert.match(reps, /Blocked by fit/);
  assert.match(reps, /Contacted/);
  assert.match(reps, /Replied/);
  ...
  assert.match(reps, /not exists \(/);
  assert.match(reps, /p\.properties #>> '\{contact_fit,decision\}' is null/);
  assert.match(reps, /p\.properties #>> '\{contact_fit,decision\}' = 'not_fit'/);
  assert.match(reps, /m\.status in \('draft','queued','deferred'\)/);
  assert.match(reps, /m\.status in \('sent','delivered','replied'\)/);
  assert.match(reps, /m\.direction = 'inbound'/);
  ...
  assert.match(reps, /Verified email/);
  assert.match(reps, /Email found/);
  assert.match(reps, /Message replied/);
  ...
  assert.match(reps, /Last-hour activity/);
  assert.match(reps, /active Agent run/);
  assert.match(reps, /activity event/);
  ...
  assert.match(reps, /Agent run advanced/);
  assert.match(reps, /Activity recorded/);
  ...
  assert.doesNotMatch(reps, /Processing \$\{activity\.active_workflows\} active workflow/);
  assert.doesNotMatch(reps, /No active workflow right now/);
  assert.doesNotMatch(reps, /Workflow advanced/);
  assert.doesNotMatch(reps, /System check completed/);
  assert.match(reps, /AgentOperatingLoopPanel/);
  assert.match(reps, /AgentCommandStrip/);
  assert.match(reps, /AgentSetupSnapshot/);
  assert.match(reps, /loadAgentSignalMix/);
  assert.match(reps, /signalMix=\{state\.signalMix\}/);
  assert.match(reps, /Agent setup/);
  assert.match(reps, /Signal mix, contact proof, and outreach readiness/);
  assert.match(reps, /Buyer fit creates source checks/);
  assert.match(reps, /Signal mix, last 7 days/);
  ...
  assert.match(reps, /Fit gate/);
  assert.match(reps, /Watched signals/);
  assert.match(reps, /6 gates ready/);
  assert.doesNotMatch(reps, /GojiBerry makes signal output concrete/);
  assert.match(reps, /aria-label="Agent launch commands"/);
  assert.match(reps, /Qualified signals/);
  assert.match(reps, /drafts ready/);
  assert.match(reps, /Connect Outlook/);
  assert.match(reps, /Email ready/);
  assert.match(reps, /Connect LinkedIn/);
  assert.match(reps, /LinkedIn ready/);
  assert.match(reps, /email_replies_7d/);
  assert.match(reps, /linkedin_messages_7d/);
  assert.match(reps, /linkedin_invites_7d/);
  assert.match(reps, /linkedin_accepts_7d/);
  assert.match(reps, /follow-ups due/);
  assert.match(reps, /accepted LinkedIn contact/);
  assert.match(reps, /the next DM/);
  assert.match(reps, /DMs 7d/);
  assert.match(reps, /Prepare outreach/);
  assert.match(reps, /Start Agent/);
  assert.match(reps, /<BrandIcon name="microsoft"/);
  assert.match(reps, /<BrandIcon name="linkedin"/);
  assert.match(reps, /Outlook unlocks native email threads and reply sync/);
  assert.match(reps, /LinkedIn unlocks connection requests, DMs, and reply capture/);
  assert.match(reps, /commandBlockerCopy/);
  assert.match(reps, /Connect Outlook or LinkedIn before outreach can run/);
  assert.doesNotMatch(reps, /readiness\.blockers\[0\] \?\? "Finish Profile before outreach can run\."/);
  ...
  assert.match(reps, /outreach when the destination account is connected and capped/);
  ...
  assert.match(reps, /coalesce\(p\.emails, '\{\}'::text\[\]\) as emails/);
  assert.match(reps, /coalesce\(m\.channel::text, a\.payload->>'channel'\) as channel/);
  assert.match(reps, /\(a\.payload->>'eval_score'\) ~ '\^\[0-9\]\+\(\\\\\.\[0-9\]\+\)\?\$'/);
  assert.match(reps, /coalesce\(m\.eval_passed, true\) as eval_passed/);
  ...
  assert.match(reps, /ReviewQualityPill/);
  assert.match(reps, /approval\.emails\.length > 0/);
  assert.match(reps, /approval\.linkedin_url \? "LinkedIn profile" : "LinkedIn pending"/);
  assert.match(reps, /numberPayload\(payload, "daily_cap"\)/);
  assert.match(reps, /numberPayload\(payload, "sent_today"\)/);
  assert.match(reps, /channel \? channelLabel\(channel\) : reviewKindLabel\(approval\.kind\)/);
  assert.match(reps, /approvalLabel\(policy\)/);
  ...
  assert.match(reps, /from "@\/core\/product\/output-destinations\.ts"/);
  assert.match(reps, /outputDestinations: buildOutputDestinations/);
  assert.match(reps, /<AgentOutputHandoffPanel destinations=\{state\.outputDestinations\} \/>/);
  assert.match(reps, /id="output-handoff" className="scroll-mt-28"/);
  assert.match(reps, /Where qualified work can go/);
  ...
  assert.match(reps, /destination\.handoff_stage === "signal_intake"/);
  assert.match(reps, /"qualified_contact_sync"/);
...
test("visitor de-anonymization enters the Signal path", () => {
  ...
  assert.match(route, /industry/);
  assert.match(route, /headcount/);
  assert.match(route, /funding_stage/);
  assert.match(route, /linkedin_url/);
  assert.match(route, /weighted_pages/);
  assert.match(route, /dwell_time_seconds/);
  assert.match(route, /scroll_depth/);
  assert.match(route, /repeat_visits/);
  assert.match(route, /consent/);
  assert.match(route, /marketing_allowed/);
  assert.match(route, /do_not_track/);
...
test("Profile exposes profile, activation, Outlook, and workspace autonomy controls", () => {
  ...
  assert.match(settings, /ProfileActivationFlow/);
  assert.match(settings, /Setup hub/);
  assert.match(settings, /Activation flow/);
  assert.match(settings, /From company profile to sent outreach/);
  assert.match(settings, /Website profile/);
  assert.match(settings, /Signal sources/);
  assert.match(settings, /Verified contacts/);
  assert.match(settings, /Channels/);
  assert.match(settings, /Outreach/);
  assert.match(settings, /readinessCheck\(readiness, "signal_sources"\)/);
  assert.match(settings, /readinessCheck\(readiness, "plays"\)/);
  assert.match(settings, /Connect Outlook or LinkedIn so qualified contacts can become outreach/);
  assert.match(settings, /The Agent can prepare judged email or LinkedIn outreach from qualified signals/);
  assert.match(settings, /Company profile/);
  assert.match(settings, /Agent and buyer fit/);
  assert.match(settings, /Contact quality/);
  ...
  assert.match(settings, /product\.source\.configure adapter=webhook provider=rb2b/);
  ...
  assert.match(settings, /page paths, dwell time, repeat visits, and consent proof/);
  ...
  assert.match(settings, /Copilot waits for approval before outreach/);
  assert.match(settings, /Autopilot can send after evals and channel checks pass/);
  assert.match(settings, /row_number\(\) over/);
  assert.match(settings, /properties ->> 'mailbox_email'/);
