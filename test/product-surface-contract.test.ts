...
test("Profile exposes profile, activation, Outlook, and workspace autonomy controls", () => {
  ...
  assert.match(settings, /src="https:\/\/www\.bombsell\.com\/visitor\.js"/);
  assert.match(settings, /window\.bombsell\('identify'\)/);
  assert.match(settings, /\/api\/collect\/visitors/);
  assert.match(settings, /product\.source\.configure adapter=webhook provider=bombsell_script website_url=https:\/\/example\.com/);
