...
Remote MCP OAuth discovery and PKCE browser token issuance are now wired for Claude Code. Bombsell serves protected-resource and authorization-server metadata under `/.well-known/*`, unauthenticated `/api/mcp` responses advertise that metadata URL in `WWW-Authenticate`, and `/api/mcp/oauth/*` can register clients, collect user consent through direct Google login when needed, and issue hashed Bombsell MCP bearer tokens.

The Claude Code distribution path now includes a concrete private marketplace catalog at `integrations/bombsell-claude-code-marketplace`. It can be validated with `claude plugin validate`, installs the focused Bombsell plugin as `bombsell@bombsell`, and pins the monorepo plugin source through Claude Code's `git-subdir` marketplace source so design-partner dogfood has an auditable install path.
