...
The Claude Code distribution path now has a concrete plugin package under `integrations/bombsell-claude-code`. It ships the Bombsell plugin manifest, remote HTTP MCP config, README, and six focused skills for Brief, Profile-from-repo, launch check, signal review, prepare-only outreach, and reply insights. The v1 package intentionally omits hooks, local binaries, and subagents until remote MCP OAuth and audit logging are complete.

Remote MCP OAuth discovery is now wired for Claude Code. Bombsell serves protected-resource and authorization-server metadata under `/.well-known/*`, and unauthenticated `/api/mcp` responses advertise that metadata URL in `WWW-Authenticate`; browser consent and token issuance remain the next backend step before public plugin auth is complete.
