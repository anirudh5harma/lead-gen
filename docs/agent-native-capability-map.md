...
| Record campaign Outcome | `/dashboard/campaigns`, MCP/internal Rep execution | `product.campaign.outcome.record` | Emits `outcome.recorded` for a campaign Play run with Play, Rep, pattern, and exemplar attribution so existing Outcome -> procedural memory learning applies | Ready |
