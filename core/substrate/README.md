...
- `events/` — typed event registry (Zod), publisher, in-memory dev bus, Postgres durable bus with supervised `LISTEN` reconnect and leased projection catch-up, and a NATS JetStream contract for future scale-out.
- `workflows/` — durable workflow runtime contracts (Step, Workflow, Checkpoint, ApprovalGate). Dev adapter runs in-process; product deployments select `BOMBSELL_SUBSTRATE=nats_restate` for Restate ingress, while `postgres` remains the local/dev bridge.
