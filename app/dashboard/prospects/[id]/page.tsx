...
interface ProfileConversationRow {
  id: string;
  status: string;
  topic: string | null;
...
interface ProspectProfileState {
  ...
  conversations: ProfileConversationRow[];
  outcomes: ProfileOutcomeRow[];
  channelState: ProfileChannelState;
}

async function loadProspectProfile(
  workspaceId: string,
  prospectId: string,
): Promise<ProspectProfileState | null> {
  const pool = getPool();
  const prospectResult = await pool.query<ProspectProfileRow>(
    `select p.id,
            p.full_name,
            p.title,
  ...
  const [signals, conversations, outcomes, channels] = await Promise.all([
    pool.query<ProfileSignalRow>(
      `select s.id,
        ...
        limit 12`,
      [workspaceId, prospect.id, prospect.company_id],
    ),
    pool.query<ProfileConversationRow>(
      `select c.id,
        ...
        limit 10`,
      [workspaceId, prospect.id],
    ),
    pool.query<ProfileOutcomeRow>(
      `select o.id,
              o.kind::text as kind,
...
export default async function ProspectProfilePage({
  ...
  const state = await loadProspectProfile(workspace.id, id);
  if (!state) return notFound();

  const { prospect, signals, conversations, outcomes, channelState } = state;
  const company = prospect.company_name ?? prospect.company_domain ?? "Unknown company";
  const reachable =
    prospect.emails.length > 0 ||
    Boolean(prospect.linkedin_url) ||
    ...
    <div className="space-y-10">
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <ReadinessTile
          icon="mail"
          label="Email"
          value={prospect.emails[0] ?? "Missing"}
          ready={prospect.emails.length > 0 && channelState.connected_outlook > 0}
          detail={
            prospect.emails.length > 0
        ...
        <ReadinessTile
          ...
          label="Company"
          value={company}
          ready={Boolean(prospect.company_name || prospect.company_domain)}
          detail={prospect.company_industry ?? prospect.company_size_bucket ?? "Graph context"}
        />
        <ReadinessTile
          icon="sensors"
          label="Timing"
...
function SignalRow({ signal }: { signal: ProfileSignalRow }) {
  return (
...
function ConversationRow({
  ...
  conversation: ProfileConversationRow;
}) {
  return (
    <Link
...
function ProfilePanel({ prospect }: { prospect: ProspectProfileRow }) {
  return (
...
function CompanyPanel({ prospect }: { prospect: ProspectProfileRow }) {
  return (
    <div className="section-note">
      <p className="text-sm font-semibold text-[var(--color-text-1)]">
        Company context
      ...
      <div className="mt-4 grid gap-3">
        <Fact label="Company" value={prospect.company_name ?? "Unknown"} icon="add_business" />
        <Fact label="Domain" value={prospect.company_domain ?? "Missing"} icon="travel_explore" />
        <Fact label="Industry" value={prospect.company_industry ?? "Unspecified"} icon="fact_check" />
        <Fact label="Size" value={prospect.company_size_bucket ?? "Unknown"} icon="person" />
      </div>
