...
/**
 * Owner-triggered redrive of one dead-lettered NATS dispatch. Resets
 ...
 * also emits `event.dispatch.redriven` so owner recovery stays auditably
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  // After redrive, send the owner back to Health.
  return Response.redirect(new URL("/dashboard/health", req.url), 303);
}
