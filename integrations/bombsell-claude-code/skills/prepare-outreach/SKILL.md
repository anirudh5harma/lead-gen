...
1. Call `bombsell_signals_list_qualified` first so the user can see the current signal/contact state.
...
3. After confirmation, call `bombsell_outreach_prepare` with `confirm_prepare=true`.
4. Report:
...
5. If drafts are ready, call `bombsell_approvals_list` and point the user to the approval gate.

Never call `bombsell_approvals_decide` unless the user explicitly asks to approve or reject a specific gate. Never imply that preparation sends outreach.
