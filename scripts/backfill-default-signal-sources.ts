#!/usr/bin/env node
/**
 * Reconcile the default free Signal source mix for existing workspace company
 * profiles. This replays the current architecture-aligned source configuration
 * path so restored Google News queries and curated RSS feeds land on live
 * workspaces without hand-editing source rows.
 *
 * Usage:
 *   npm run backfill:default-signal-sources -- --dry-run
 *   npm run backfill:default-signal-sources -- --limit=10
 *   npm run backfill:default-signal-sources -- --workspace=<workspace_id>
 */
