...
async function sendAutomationLead(
  ...
  const leadCompany = String(params.lead.target_company ?? 'lead')
  let graph: LeadGraphResult | null = null
  let draftQualityScore: number | null = null
  ...
  try {
    ...
    const graphStepId = await startWorkflowStep(supabase, {
      ...
      userId: params.userId,
      clientId: params.clientId,
      stepKey: `lead:${leadId}:graph_sync`,
      input: { lead_id: leadId },
    })
    graph = await upsertLeadIntoGtmGraph(supabase, {
      id: leadId,
      user_id: params.userId,
      client_id: params.clientId,
      target_company: leadCompany,
    ...
    const planStepId = await startWorkflowStep(supabase, {
      ...
      userId: params.userId,
      clientId: params.clientId,
      stepKey: `lead:${leadId}:outreach_plan`,
      input: { lead_id: leadId, mode: 'autonomous' },
    })
    if (graph) {
    ...
    if (outreachPlan.requires_review) {
      ...
      await recordAgentEvent(supabase, {
        ...
        status: 'blocked',
        title: `Skipped ${leadCompany}: plan needs review`,
      ...
      await upsertGtmEvalTrace(supabase, {
        ...
        clientId: params.clientId,
        leadId,
        traceType: 'automation_send',
        status: 'blocked',
        reasons: reasons.length > 0 ? reasons : ['outreach_plan_needs_review'],
        metadata: { blocked_at: 'outreach_plan', outreach_plan: outreachPlan, workflow_run_id: params.workflowRunId },
      })
      await recordAutomationBlockerMemory(supabase, {
        userId: params.userId,
        clientId: params.clientId,
        leadId,
        graph,
    ...
    if (!draft.recipientGroup) {
      await upsertGtmEvalTrace(supabase, {
        ...
        traceType: 'automation_send',
        status: 'blocked',
        reasons: ['no_valid_recipients'],
        metadata: { blocked_at: 'draft', workflow_run_id: params.workflowRunId },
      })
      await recordAutomationBlockerMemory(supabase, {
        userId: params.userId,
        clientId: params.clientId,
    ...
    const draftQuality = evaluateOutreachDraftQuality({
      ...
      body: draft.body,
      greeting: draft.recipientGroup.greeting,
      targetCompany: leadCompany,
      signalType: signal?.signal_type ?? null,
      signalSummary: signal?.summary ?? (params.lead.relevance_reason as string | null | undefined) ?? null,
    })
    draftQualityScore = draftQuality.score
    if (draftQuality.score < 55) {
      const reasons = ['draft_quality_low', ...draftQuality.failed]
      await recordAgentEvent(supabase, {
        userId: params.userId,
        clientId: params.clientId,
        leadId,
        ...
        agentName: 'message',
        eventType: 'draft_quality_blocked',
        status: 'blocked',
        title: `Skipped ${leadCompany}: draft quality too low`,
        body: 'Autopilot paused this lead because the generated draft failed quality checks.',
        metadata: {
          quality_score: draftQuality.score,
          failed_checks: draftQuality.failed,
      ...
      await upsertGtmEvalTrace(supabase, {
        ...
        reasons,
        draftQualityScore: draftQuality.score,
        metadata: { blocked_at: 'draft_quality', quality_version: draftQuality.version, workflow_run_id: params.workflowRunId },
      })
      await recordAutomationBlockerMemory(supabase, {
        userId: params.userId,
        clientId: params.clientId,
    ...
    const validationStepId = await startWorkflowStep(supabase, {
      ...
      userId: params.userId,
      clientId: params.clientId,
      stepKey: `lead:${leadId}:recipient_validation`,
      input: {
    ...
    if (!recipientValidation.safe) {
      ...
      await recordAgentEvent(supabase, {
        ...
        status: 'blocked',
        title: `Skipped ${leadCompany}: recipient validation failed`,
      ...
      await upsertGtmEvalTrace(supabase, {
        ...
        reasons,
        draftQualityScore: draftQuality.score,
        metadata: { blocked_at: 'recipient_validation', unsafe_emails: recipientValidation.unsafeEmails, workflow_run_id: params.workflowRunId },
      })
      await recordAutomationBlockerMemory(supabase, {
        userId: params.userId,
        clientId: params.clientId,
    ...
    if (policyDecision.decision !== 'allowed') {
      await recordAgentEvent(supabase, {
        ...
        status: 'blocked',
        title: `Skipped ${leadCompany}: policy blocked send`,
      ...
      await upsertGtmEvalTrace(supabase, {
        ...
        reasons: policyDecision.reasons,
        draftQualityScore: draftQuality.score,
        metadata: { blocked_at: 'policy', workflow_run_id: params.workflowRunId },
      })
      await recordAutomationBlockerMemory(supabase, {
        userId: params.userId,
        clientId: params.clientId,
    ...
    if (!result) {
      ...
      await upsertGtmEvalTrace(supabase, {
        ...
        draftQualityScore: draftQuality.score,
        metadata: { blocked_at: 'send', workflow_run_id: params.workflowRunId },
      })
      await recordAutomationBlockerMemory(supabase, {
        userId: params.userId,
        clientId: params.clientId,
    ...
    await recordAgentEvent(supabase, {
      ...
      status: 'completed',
      title: `Sent outreach to ${leadCompany}`,
    ...
    const message = error instanceof Error ? error.message : 'automation send failed'
    await upsertGtmEvalTrace(supabase, {
      userId: params.userId,
      clientId: params.clientId,
      leadId,
      ...
      draftQualityScore,
      metadata: { failed_at: 'send_automation_lead', workflow_run_id: params.workflowRunId },
    })
    await recordAutomationBlockerMemory(supabase, {
      userId: params.userId,
      clientId: params.clientId,
    ...
    await recordAgentEvent(supabase, {
      ...
      leadId,
      agentName: 'sending',
      eventType: 'automation_send_failed',
      status: 'blocked',
      title: `Failed send for ${leadCompany}`,
      body: message,
    })
    return false
  }
...
async function recordAutomationBlockerMemory(
  supabase: Awaited<ReturnType<typeof createServiceClient>>,
  input: {
    userId: string
    clientId: string | null
    leadId: string
    graph: LeadGraphResult | null
    reasons: string[]
    source: string
    workflowRunId: string | null
    agentRunId: string | null
  ...
  await recordGtmMemory(supabase, {
    ...
    memoryType: 'execution_blocker',
    content: `Execution blocked for lead ${input.leadId}: ${input.reasons.join(', ')}.`,
    source: input.source,
    confidence: 1,
    observedAt: new Date().toISOString(),
    provenance: {
